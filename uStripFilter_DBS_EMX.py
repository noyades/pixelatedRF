...
pathName = 'H:\\Shared drives\\RRFC\\SimFiles\\dbsEmx\\' # Base path for file creation

w_l, l_l = MicrostripCalc.synth_microstrip(sub1, z0, 2)
print('quarter wave line=',25.4*w_l,25.4*l_l)
w_90, l_90 = MicrostripCalc.synth_microstrip(sub1, z0, 30)
...
sym = 'asym'
x = datetime.now()
rrfc1 = RandomComponent(unit=layoutUnit,ports=ports,sides=sides,corner=corner,connect=connectMap,\
...
csv_file = 0#'/home/jswalling/pythonWork/rrfc/dbsEmx/data/twoPort_random_pixelSize=' + str(refPixelSize) + '_start.csv'
gds_file = 'H:\\Shared drives\\RRFC\\SimFiles\\dbsEmx\\data\\twoPort_random_pixelSize=' + str(pixelSize) + '_init.gds'
...
if csv_file != 0:
  ...
  portPosition, _, _, _, _ = rrfc1.random_gds_dim(sub1, xProto*pixelSize, yProto*pixelSize, z0)
else:
  reSizePixMap = False
...
def cost_function(pixMap):
  ...
  cell = rfc2.create_gds_from_pixmap_file(sub1,50,'none')  
