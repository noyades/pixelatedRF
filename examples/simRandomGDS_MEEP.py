...
simulator = 'MEEP' # This controls the simulation to be used. Right now there are two valid values 'ADS' or 'EMX'
...
rfc1 = rfc(unit=layoutUnit,corner='standard',pixelSize=pixelSize,layoutRes=layoutRes,scale=scale,sim=simulator,\
...
y = 1
for x in range(0,2): # Run 100 iterations of file generation and simulation.
  random.seed(x)
  ...
  rrfc1 = rrfc(unit=layoutUnit,ports=ports,sides=sides,corner='standard',connect=connectMap,\
          minPix=minPixel,pixelSize=pixelSize,layoutRes=layoutRes,scale='',launchLen=30,seed=x,\
          sim=simulator,view=view,write=write,outF=data_file,sym=sym,portPosition='')
  ...
  if sim == True:
    ...
    em1 = emSim(workingPath = pathName, adsLibName = libName, gdsFile = gds_file,\
              ...
              gdsCellName = cell, dataFile = data_file, Substrate=sub1)
    emSim.meepRun(em1)
