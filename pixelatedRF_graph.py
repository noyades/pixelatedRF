...
def main():
  ...
  fc = init_vars.get("fc", 8.0e9) # Operating center frequency for electrical length calculations
            # (make this smaller than or equal to the desired operating frequency
  z0 = init_vars.get("z0", 50) # Desired characteristic impedance of launches
  EL = init_vars.get("EL", 90) # Desired unit of electrical length in degrees
  t = init_vars.get("t", 1.4) # Thickness of conductor in mils
  cond = init_vars.get("cond", 5.88e7) # Conductivity of the conductors
  h = init_vars.get("h", 30) # height of conductor above substrate
  t_air = init_vars.get("t_air", 2*(t+h)) # thickness of the air above the conductor layer
  er = init_vars.get("er", 3.66) # relative permittivity of the substrate material

  sub1 = MicrostripSub(t, cond, h, er, fc)

  simulator = init_vars.get('simulator', 'ADS') # This controls the simulation to be used. Right now there are two valid values 'ADS' or 'EMX'
  simulatorPath = init_vars.get('simulatorPath', 'C:\Program Files\Keysight\ADS_2023_1\bin') # This is the path to the simulator executable
  libName = init_vars.get('libName', 'dbsEmSim') # This is the name of the ADS workspace that was created to run sims in
  sim = init_vars.get('sim', True) # This controls whether a simulation is run or not.
  view = init_vars.get('view', True) # This controls whether a simulation is run or not.
  write = init_vars.get('write', True) # Control whether output files are written or not
  ports = init_vars.get('ports', 2) # For now, code makes either 2, 3 or 4 ports
  sides = init_vars.get('sides', 2) # For now, code can put ports on 2, 3 or 4 sides, with constraints that are spelled out in rrfc
  shape = init_vars.get('shape', 'rect') # This is the shape of the layout. Options are 'rect' or 'square'. Square is a special case of rectangular.
  corner = init_vars.get('corner', 'normal') # Options are 'overlap' or 'noverlap'=non-overlap. Use overlap to predict what will happen
                    # in corners due to manufacturing, or noverlap to guarantee non-overlap at corners
  scale = init_vars.get('scale', 1) # This is a scale factor for the pixel size. It is used to scale the pixel size and minimum pixel size.
  # The pixel size is the size of the pixel that will be used to create the layout. It is typically constrained by the PCB manufacturer.
  ...
  pathName = init_vars.get('pathName', 'C:\\Users\\Owner\\Documents\\dbs_optimization\\') # Base path for file creation
  ...
  def cost_function(pixMap):
    ...
    em1 = EmSim(workingPath = pathName, adsLibName = libName, gdsFile = gds_file2,\
                ...
                gdsCellName = cell, dataFile = outFile, simPath = simulatorPath)
    em1.mom_run()
