...
class UStrip:
  def __init__(self, 
    ...
    Define the microstrip substrate.
        ...
        Args:
            ...
            corner (str): How the corner connection is realized for simulation.
            pixelSize (int): Size of the randomized pixel.
            layoutRes (int): Layout resolution.
            scale (int): Scale factor.
            sim (str): Simulator to assume (this is mainly for port placement and setup).
            view (bool): Flag to view GDS (do not use in sim mode).
            write (bool): Flag to write files.
            outF (str): Output file string.
    """    
    self.unit = unit
    self.corner = corner
  ...
  def _calc_launch_pixels(self, sub, imp: float):
    """
        Calculate the number of pixels required for the launch.

        Args:
            sub (MicrostripSub): The microstrip substrate.
            imp (float): Desired impedance.

        Returns:
            tuple: A tuple containing the number of pixels in the x direction, 
                   the number of pixels in the y direction, and the launch length.
    """
    width_launch, length_launch = MicrostripCalc.synth_microstrip(sub, imp, 5)
    launch_x_pixels = round(self.scale * length_launch * self.mil2um / self.pixelSize) # length
    launch_y_pixels = round(self.scale * width_launch * self.mil2um / self.pixelSize) # length of the line to connect to structure in number of pixels
    length_launch = launch_x_pixels * self.pixelSize / self.scale
    return launch_x_pixels, launch_y_pixels, length_launch

  def u_strip_quad_hybrid_gds(self, sub, imp: float):
    ...
    width_ref, length_ref = MicrostripCalc.synth_microstrip(sub, imp, 30)
    width_Zo, length_Zo = MicrostripCalc.synth_microstrip(sub, imp, 5)
    width_Zl, length_Zl = MicrostripCalc.synth_microstrip(sub, imp / np.sqrt(2), 5)
    numHorSections = np.around(length_ref / length_Zl)
    numVerSections = np.around(length_ref / length_Zo)
    hor_x_pixels = round(self.scale * length_Zl * self.mil2um / self.pixelSize)
    hor_y_pixels = round(self.scale * width_Zl * self.mil2um / self.pixelSize)
    ver_x_pixels = round(self.scale * width_Zo * self.mil2um / self.pixelSize)
    ver_y_pixels = round(self.scale * length_Zo * self.mil2um / self.pixelSize)

    # Calculate Port Dimenstions
    launch_x_pixels, launch_y_pixels, length_launch = self._calc_launch_pixels(sub, imp)
    ...
    def draw_section(x_start, y_start, x_pixels, y_pixels, horizontal=True):
      for x in range(x_pixels):
        for y in range(y_pixels):
          if horizontal:
            pixMap[x_start + x, y_start + y] = 1
          else:
            pixMap[y_start + y, x_start + x] = 1
      return pixMap
      
    pixMap = np.zeros((int(x_pixels), int(y_pixels)), dtype=int)
    # Add launches: assume a rectangle with port 1 = NorthWest
    draw_section(0, int(y_pixels - buf_pixels - launch_y_pixels), launch_x_pixels, launch_y_pixels)
    ...
    draw_section(0, buf_pixels, launch_x_pixels, launch_y_pixels)
    ##for x in range(launch_x_pixels):
    ##  for y in range(launch_y_pixels):
    ...
    draw_section(int(x_pixels - launch_x_pixels), int(y_pixels - buf_pixels - launch_y_pixels), launch_x_pixels, launch_y_pixels) 
    ##for x in range(launch_x_pixels):
    ##  for y in range(launch_y_pixels):
    ...
    draw_section(int(x_pixels - launch_x_pixels), buf_pixels, launch_x_pixels, launch_y_pixels) 
    ##for x in range(launch_x_pixels):
    ##  for y in range(launch_y_pixels):
    ...
    if self.sim == 'EMX':
      ly = pya.Layout()
      ly.dbu = self.unit * 1e6 # Set the database unit to 1 um. Generally for EMX it is easier to work in um.
      UNIT = ly.create_cell(cellName) # Create Cell obj
      # Create layer #'s
    ...
    if self.corner == 'overlap':
        for x in range(0, cols):
          for y in range(0, rows):
            if pixMap[y,x]  == 1:
              rect = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(x * self.pixelSize, y * self.pixelSize)
  ...
  def u_strip_stepped_imp_filter_gds(self, sub, filtType: str, order: int, w_h: int, w_l: int, Zo_h: float, Zo_l: float, imp: float):
    ...
    for filt in filtElecLength:
      if x % 2 == 0:
        widthSec[x], lengthSec[x] = MicrostripCalc.synth_microstrip(sub, Zo_l, filt)
      else:
        widthSec[x], lengthSec[x] = MicrostripCalc.synth_microstrip(sub, Zo_h, filt)
      if self.sim == 'EMX':
        y_pixels[x] = np.around(25.4 * widthSec[x])
        x_pixels[x] = np.around(25.4 * lengthSec[x])
      else:
    ...
    # Pixel Size
    # Calculate Port Dimenstions
    launch_l_pixels, launch_w_pixels, length_launch = self._calc_launch_pixels(sub, imp)
    ...
    if self.sim == 'EMX':
      ...
      for x in range(launch_l_pixels):
        for y in range(launch_w_pixels):
          launch_1 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(x * self.pixelSize, \
                   (y + math.ceil(int(y_dim / 2)) - math.floor(launch_w_pixels / 2)) * self.pixelSize)
          pixel_map[x, y + math.ceil(int(y_dim / 2)) - math.floor(launch_w_pixels / 2)] = 1
          launch_2 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(x_total - \
                     length_launch * self.scale + x * self.pixelSize, (y + math.ceil(int(y_dim / 2)) - \
                     math.floor(launch_w_pixels / 2)) * self.pixelSize)
      ...
      for z in range(len(coeffs)):
        for x in range(int(x_pixels[z])):
          for y in range(int(y_pixels[z])):
            rect = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(length_launch * self.scale + \
                   x * self.pixelSize + int(sum(x_pixels[0:z]) * self.pixelSize), (y + math.ceil(int(y_dim / 2)) - \
                   math.floor(int(y_pixels[z]) / 2)) * self.pixelSize)
      ...
      for x in range(launch_l_pixels):
        for y in range(launch_w_pixels):
          launch_1 = gdspy.Rectangle((0, 0), (self.pixelSize, self.pixelSize), **l_top).translate(x*self.pixelSize, \
                     (y+math.ceil(int(y_dim/2))-math.floor(launch_w_pixels/2))*self.pixelSize)
          pixel_map[x,y+math.ceil(int(y_dim/2))-math.floor(launch_w_pixels/2)] = 1
          launch_2 = gdspy.Rectangle((0, 0), (self.pixelSize, self.pixelSize), **l_top).translate(x_total - \
                     length_launch + x*self.pixelSize, (y+math.ceil(int(y_dim/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize)
          pixel_map[int(x_dim) - launch_l_pixels + x, y+math.ceil(int(y_dim/2)) - \
                    math.floor(launch_w_pixels/2)] = 1
          UNIT.add(launch_1)
      ...
      for z in range(len(coeffs)):
        for x in range(int(x_pixels[z])):
          for y in range(int(y_pixels[z])):
            rect = gdspy.Rectangle((0, 0), (self.pixelSize, self.pixelSize), **l_top).translate(length_launch + \
                   x*self.pixelSize + int(sum(x_pixels[0:z])*self.pixelSize), (y+math.ceil(int(y_dim/2)) - \
                   math.floor(int(y_pixels[z])/2))*self.pixelSize)
            pixel_map[launch_l_pixels + x + int(sum(x_pixels[0:z])),y+math.ceil(int(y_dim/2)) - \
                   math.floor(int(y_pixels[z])/2)] = 1
  ...
  def u_strip_tee_junction_gds(self, sub, x_pixels: float, y_pixels: float, imp: float):
    ...
    if self.sim == 'EMX':
      ...
      # Add the filter elements
      widthSec, lengthSec = MicrostripCalc.synth_microstrip(sub, imp, 90)
      ...
      for x in range(tee_l_pixels):
        for y in range(tee_w_pixels):
          rect1 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(length_launch + \
                 x*self.pixelSize, (y+math.ceil(int((y_dim-launch_l_pixels)/2))-math.floor(launch_w_pixels/2))*self.pixelSize)
      ...
      for x in range(tee_w_pixels):
        for y in range(int((y_pixels - tee_w_pixels)/2)):
          rect2 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved((x+math.ceil(int(x_dim/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize, (y+math.ceil(int((y_dim-launch_l_pixels)/2)) + \
                     math.floor(launch_w_pixels/2)+1)*self.pixelSize)
      ...
      for x in range(launch_l_pixels):
        for y in range(launch_w_pixels):
          launch_1 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(x*self.pixelSize, \
                   (y+math.ceil(int((y_dim-launch_l_pixels)/2))-math.floor(launch_w_pixels/2))*self.pixelSize)
          pixel_map[x,y+math.ceil(int((y_dim-launch_l_pixels)/2))-math.floor(launch_w_pixels/2)] = 1
          launch_2 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved(x_total - \
                     length_launch + x*self.pixelSize, (y+math.ceil(int((y_dim-launch_l_pixels)/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize)
          pixel_map[int(x_dim) - launch_l_pixels + x, y+math.ceil(int((y_dim-launch_l_pixels)/2)) - \
                    math.floor(launch_w_pixels/2)] = 1
          launch_3 = pya.Box(0, 0, self.pixelSize, self.pixelSize).moved((y+math.ceil(int(x_dim/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize, y_total - \
                     length_launch + x*self.pixelSize)
      ...
      for x in range(tee_w_pixels):
        for y in range(int((y_pixels - tee_w_pixels)/2)):
          rect = gdspy.Rectangle((0, 0), (self.pixelSize, self.pixelSize), **l_top).translate((x+math.ceil(int(x_dim/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize, (y+math.ceil(int((y_dim-launch_l_pixels)/2)) + \
                     math.floor(launch_w_pixels/2)+1)*self.pixelSize)
          pixel_map[(x+math.ceil(int(x_dim/2)) - math.floor(launch_w_pixels/2)),(y+math.ceil(int((y_dim-launch_l_pixels)/2)) + \
                     math.floor(launch_w_pixels/2)+1)] = 1
      ...
      for x in range(launch_l_pixels):
        for y in range(launch_w_pixels):
          ...
          pixel_map[int(x_dim) - launch_l_pixels + x, y+math.ceil(int((y_dim-launch_l_pixels)/2)) - \
                    math.floor(launch_w_pixels/2)] = 1
          launch_3 = gdspy.Rectangle((0, 0), (self.pixelSize, self.pixelSize), **l_top).translate((y+math.ceil(int(x_dim/2)) - \
                     math.floor(launch_w_pixels/2))*self.pixelSize, y_total - \
                     length_launch + x*self.pixelSize)
          pixel_map[(y+math.ceil(int(x_dim/2)) - \
                     math.floor(launch_w_pixels/2)), y_dim - launch_l_pixels + x] = 1
